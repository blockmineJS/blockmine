...
5. Загрузка: `reload_plugin`, потом `get_bot_plugins`. Поле `loadError` — ошибка `onLoad`. `null` значит, что плагин загрузился. `isEnabled: true` при непустом `loadError` значит, что плагин включён, но код не встал. Бот при этом жив, остальные плагины загружены. Подробный stack есть в `get_bot_logs` строкой `[PluginLoader] [FATAL]`. Игровую проверку сам не запускай, см. раздел «Проверка».
...
Бросок или reject из `onLoad` не роняет процесс бота. Панель пишет ошибку в `loadError`, плагин остаётся включённым. После правки снова `reload_plugin` и проверь, что `loadError` стал `null`.

Типа настройки `json` нет. Для объекта указывай `json_file`. `default` может быть самим объектом, `defaultPath` — файл внутри папки плагина.
...
Права из `botpanel.permissions` панель создаёт сама при установке, при запуске бота и при перечитывании плагина. Уже существующее право с тем же именем не перезаписывается. Повторно создавать их в `onLoad` не нужно. `registerCommand` по-прежнему сам создаёт право команды, если его ещё нет.
