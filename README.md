...
*   **🔌 WebSocket API для интеграций**:
    *   Управляйте ботами из внешних приложений через WebSocket API.
    *   Выполняйте команды бота от имени любого пользователя с полной проверкой прав.
    *   Вызывайте визуальные графы и получайте результаты их выполнения.
    *   Подписывайтесь на события бота (сообщения в чате, вход/выход игроков, здоровье и др.).
    *   Официальный SDK `blockmine-api` для Node.js с полной документацией.
...
### 3. WebSocket API для интеграций

BlockMine предоставляет мощный WebSocket API для интеграции с внешними приложениями. Вы можете управлять ботами, вызывать графы и подписываться на события из любого языка программирования.
...
async function main() {
    ...
    // Выполнить команду бота от имени пользователя
    const result = await bot.executeCommand('Steve', 'ping');
    console.log('Результат команды:', result); // "Понг, Steve!"

    // Вызвать визуальный граф и получить результат
    const graphResult = await bot.callGraph('calculate', {
        operation: 'add',
        a: 10,
        b: 20
    });
    console.log('Результат графа:', graphResult);
}
...
Полная документация по WebSocket API доступна в пакете `blockmine-api`:
- [README.md](./packages/blockmine-api/README.md) - Полное руководство по API
- [examples/](./packages/blockmine-api/examples/) - Примеры использования

#### Возможности API

**Выполнение команд:**
```javascript
// Команды выполняются с полной проверкой прав пользователя
const result = await bot.executeCommand('Steve', 'ping');
// Результат: "Понг, Steve!"

// С аргументами
const result2 = await bot.executeCommand('Steve', 'ping', { target: 'Alex' });
// Результат: "Понг, Steve! Пингую игрока Alex."
```

**Вызов графов:**
```javascript
// Графы с триггером "Вызов из WebSocket API" возвращают данные
const data = await bot.callGraph('server_info', {});
console.log(data); // { players: [...], position: {...} }
```
...
**Универсальная система выполнения:**
- Команды через WebSocket API используют ту же валидацию, что и команды из Minecraft
- Все проверки прав, кулдаунов и черного списка работают одинаково
- Это позволяет создавать единую систему команд для разных источников (игра, API, будущий Telegram бот)
