...
- **SDK** `blockmine-api` для Node.js ⚠️ *(альфа-версия, не приоритет)*
...
> ⚠️ **Внимание**: SDK `blockmine-api` находится в **альфа-стадии** и не является приоритетом разработки. API может меняться, документация может быть неполной. Используйте на свой риск.

<td align="center">
      <p><strong>Веб сокеты</strong></p>
      <img src="./screen/websocket.png" alt="Trace Debug" width="100%">
      <em>Возможность подключать к своему тг/дс и другому внешему приложению ботов на blockmine</em>
    </td>
  </tr>
...
bot.onHealthChange((health, food) => {
    console.log(`HP: ${health}, Hunger: ${food}`);
});
```
...
- Это позволяет создавать единую систему команд для разных источников (игра, API, Telegram бот)
