...
export default function GroupsManager({ groups, allPermissions, botId, isLoading, onDataChange }) {
    ...
    const handleDelete = async (group) => {
        ...
        if (window.confirm(`Вы уверены, что хотите удалить группу "${group.name}"?`)) {
            try {
                const response = await fetch(`/api/bots/${group.botId}/groups/${group.id}`, { method: 'DELETE' });
                if (!response.ok) throw new Error((await response.json()).error || 'Server Error');
                            ...
                            groups.map(group => (
                                <TableRow key={group.id}>
                                    ...
                                    <TableCell className="text-right">
                                        ...
                                        <Button variant="ghost" size="icon" disabled={group.owner !== 'admin'} onClick={() => handleDelete(group)}><Trash2 className="h-4 w-4 text-destructive" /></Button>
                                    </TableCell>
                                </TableRow>
                            ))
                        )}
