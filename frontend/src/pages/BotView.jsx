...
import { Play, Square, Settings, Puzzle, Terminal, Trash2, Users, Download } from 'lucide-react';
...
export default function BotView() {
    ...
    if (!bot) {
        return <div className="flex items-center justify-center h-full text-muted-foreground">Загрузка данных бота...</div>;
    }
    ...
    return (
        <>
            <div className="flex flex-col h-full w-full overflow-hidden">
                ...
                <nav className="flex items-center gap-6 px-4 py-2 border-b shrink-0">
                     <NavLink to="console" className={({isActive}) => `flex items-center gap-2 text-sm pb-2 border-b-2 ${isActive ? 'border-primary' : 'border-transparent text-muted-foreground hover:text-primary'}`}><Terminal className="h-4 w-4"/>Консоль</NavLink>
                     <NavLink to="plugins" className={({isActive}) => `flex items-center gap-2 text-sm pb-2 border-b-2 ${isActive ? 'border-primary' : 'border-transparent text-muted-foreground hover:text-primary'}`}><Puzzle className="h-4 w-4"/>Плагины</NavLink>
                     <NavLink to="settings" className={({isActive}) => `flex items-center gap-2 text-sm pb-2 border-b-2 ${isActive ? 'border-primary' : 'border-transparent text-muted-foreground hover:text-primary'}`}><Settings className="h-4 w-4"/>Настройки</NavLink>
                     <NavLink to="management" className={({isActive}) => `flex items-center gap-2 text-sm pb-2 border-b-2 ${isActive ? 'border-primary' : 'border-transparent text-muted-foreground hover:text-primary'}`}><Users className="h-4 w-4"/>Управление</NavLink>
                </nav>
